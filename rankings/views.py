...
def index(request):
	...
	for p in players:
		tmp_dct = {}
		logger.info((week.id-1,week.id-2,week.id-3))
		wins = Match.objects.filter(week_id__in=(week.id-1,week.id-2,week.id-3)).filter(winner_player=p).count()
		home_cats = Match.objects.filter(week_id__in=(week.id-1,week.id-2,week.id-3)).filter(home_player=p).aggregate(cats=Sum('home_cat'))
		away_cats = Match.objects.filter(week_id__in=(week.id-1,week.id-2,week.id-3)).filter(away_player=p).aggregate(cats=Sum('away_cat'))
		if home_cats['cats'] == None:
			home_cats['cats'] = 0
		if away_cats['cats'] == None:
			away_cats['cats'] = 0
		tmp_dct['cats'] = home_cats['cats'] + away_cats['cats']
		fpfg = Team_Point.objects.filter(player=p).filter(point__date__in=Week_Date.objects.filter(week_id__in=(week.id-1,week.id-2,week.id-3)).values_list('date', flat=True)).aggregate(fantasy_points=Sum('point__fantasy_points'))
		tmp_dct['player_name'] = p.name
		tmp_dct['wins'] = wins
		tmp_dct['fpfpg'] = int(fpfg['fantasy_points']/3)
		tmp_dct['fpapg'] = 0

                for m in Match.objects.filter(week_id__in=(week.id-1,week.id-2,week.id-3)).filter(Q(home_player=p)|Q(away_player=p)).order_by('week'):
                        if m.home_player == p:
		...
		tmp_dct['fpapg'] = int(tmp_dct['fpapg']/3)
